...
def createTimeFeatures(data,features_to_create=["EPOCH","START_OF_DAY","MONTH","WEEKDAY","HOLYDAYS"], delete = True):
    """
    ...
    holydaysCalendar = createHolydaysCalendar(const.PATH_HOLYDAYS_DATES)
    appliedFunction = partial(isInHolyday,holydaysCalendar)
    ...
    features = {
        ...
        "HOLYDAYS": {'aF' : appliedFunction, 'type':'category'}
    }
...
def createCategoricalFeatures(data,feature,delete=True):
    ...
    for i in data[feature].unique():
        ...
        if (not(feature == "HOLYDAY" and str(i) =="")):
...
def createDayNightFeature(data):
    ...
    return data
"""
    Function to create new features corresponding to whether the epoch is during holydays or not.
    :param  datetime: a datetime object
            holydaysCalendar: a dataframe, corresponding to the one created by createHolydaysCalendar function.
    :return str: a string of the name of the holydays, its value is "NONE" if the argument is not within any holyday.
"""
def isInHolyday(holydaysCalendar,datetime):
    for i in holydaysCalendar.columns:
        if (datetime < holydaysCalendar[i]["DATETIME_END"] and datetime > holydaysCalendar[i]["DATETIME_BEGIN"]):
            return holydaysCalendar[i]["NAME_HOLYDAYS"]
    return "NONE"
"""
    Create a dataframe from the calendar of the holydaysCalendar
    :param  path: a string containing the path to the .csv file
    :return cal.T : a dataframe with a column per holyday.
"""
def createHolydaysCalendar(path):
